...
On startup, Workshop now inspects the host firewall rules that apply to the
LXD bridge. If forwarding is blocked, typically because of Docker, UFW, or
firewalld defaults, Workshop logs a warning and records a state warning that
points to the LXD network-bridge firewalld guide. Workshops still start, but
the operator is alerted before networking issues surface as opaque failures.
