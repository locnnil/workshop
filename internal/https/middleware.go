...
func (p RetryPolicy) Validate() error {
	if p.Attempts < 1 {
		return errors.New("expected at least one attempt")
	}
	if p.MaxDelay < 1 {
		return errors.New("expected max delay to be a valid time")
	}
	return nil
}
...
func (m retryMiddleware) defaultBackoff(resp *http.Response, backoff time.Duration) (time.Duration, error) {
	...
	if header := resp.Header.Get("Retry-After"); header != "" {
		...
		logger.Noticef("unable to parse Retry-After header %s from %s", header, url)
	}
