...
func (ovs *overlordSuite) TestTrivialRunAndStop(c *C) {
	o, err := overlord.New(ovs.dir, workshopbackend.NewFakeWorkshopBackend(), nil)
	c.Assert(err, IsNil)
...
func (ovs *overlordSuite) TestRequestRestartHandler(c *C) {
	...
	o, err := overlord.New(ovs.dir, workshopbackend.NewFakeWorkshopBackend(), rb)
	c.Assert(err, IsNil)
...
func (ovs *overlordSuite) TestVerifyRebootNoPendingReboot(c *C) {
	...
	_, err = overlord.New(ovs.dir, workshopbackend.NewFakeWorkshopBackend(), rb)
	c.Assert(err, IsNil)
