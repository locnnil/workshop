...
func (s *patchSuite) TestNoDowngrade(c *C) {
	...
	st.Unlock()
	err := patch.Apply(st)
	c.Assert(err, ErrorMatches, `cannot downgrade: workshopd is too old for the current system state \(patch level 3\)`)
}
...
func (s *patchSuite) TestMissing(c *C) {
	...
	st.Unlock()
	err := patch.Apply(st)
	c.Assert(err, ErrorMatches, `cannot upgrade: workshopd is too new for the current system state \(patch level 1\)`)
}
...
func (s *patchSuite) testMaybeResetPatchLevel6(c *C, snapdVersion, lastVersion string, expectedPatches []int) {
	...
	c.Check(ver, Equals, "workshopd-version-1")
	c.Check(level, Equals, 6)
	c.Check(sublevel, Equals, 2)
}

func (s *patchSuite) TestSameSnapdVersionLvl60PatchesNotApplied(c *C) {
	// sublevel patches not applied if workshopd version is same
	s.testMaybeResetPatchLevel6(c, "workshopd-version-1", "workshopd-version-1", nil)
}

func (s *patchSuite) TestDifferentSnapdVersionPatchLevel6NoLastVersion(c *C) {
	s.testMaybeResetPatchLevel6(c, "workshopd-version-1", "", []int{61, 62})
}

func (s *patchSuite) TestDifferentSnapdVersionPatchLevel6(c *C) {
	s.testMaybeResetPatchLevel6(c, "workshopd-version-1", "workshopd-version-2", []int{61, 62})
}
