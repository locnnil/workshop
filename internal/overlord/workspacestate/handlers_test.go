...
func (s *workspaceHandlers) TestStopPastTimeout(c *check.C) {
	s.state.Lock()
	defer s.state.Unlock()
	err := os.WriteFile(filepath.Join(s.project.Path, ".workspace.ws.yaml"), []byte(`name: ws
base: ubuntu@20.04
`), 0644)
	...
	oldInterval := workspacestate.StopLogInterval
	workspacestate.StopLogInterval = 10 * time.Second
	oldTimeout := workspacestate.StopTimeout
	workspacestate.StopTimeout = 100 * time.Millisecond
	// the fake StopWorkspace function will wait on the channel as otherwise
	// (e.g. wait for longer then StopTimeout) it will create a race between
	// restore() and a delayed call of StopWorkspace in the doStop handler
	...
	// make sure that if the stop timed out, it stopped waiting
	// and returned an appropriate error
	c.Assert(t1.Tasks()[0].Log(), check.HasLen, 1)
	c.Check(chg.Err(), check.ErrorMatches, "*.cannot stop \"ws\": timed out.*")
	workspacestate.StopLogInterval = oldInterval
	workspacestate.StopTimeout = oldTimeout
	stop <- struct{}{}
}
