...
func SetupHook(st *state.State, sdk *workshopbackend.SdkRecord, hook WorkspaceHookType) *state.Task {
	...
	if hook == SaveState || hook == RestoreState {
		setup.Environment["SDK_STATE_DIR"] = filepath.Join(workshopbackend.WorkspaceStateDir, "sdk", sdk.Name)
	}
