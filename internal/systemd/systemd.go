...
// FakeStopDelays is used from tests so that Stop can be less
// forgiving there.
...
func MountUnitPath(baseDir string) string {
	// TODO This shows why the idea of having the global "dirs" package in snapd
	//      is not so great. This package has a local "root" in the instance,
	//      and then uses a global chroot. I'm pretty sure that if those two
	//      diverge it will all fall apart. IMO it would be better if each
	//      value (not even package) took care of itself, instead of relying on
	//      global state.
	escapedPath := EscapeUnitNamePath(baseDir)
	return filepath.Join(ServicesDir, escapedPath+".mount")
}
